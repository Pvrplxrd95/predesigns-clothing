...
function proceedToPayment() {
    const cart = JSON.parse(localStorage.getItem('predesigns-cart')) || [];
...
function submitBankTransferOrder(orderNumber, totalAmount) {
    ...
    .then(response => {
        if (response.ok) {
            ...
            // Clear cart
            localStorage.removeItem('predesigns-cart');
            updateCartCount();
...
// Add Cart Button Enhancement
function enhanceCartButton() {
    const cartIcon = document.querySelector('.cart-icon');
    if (cartIcon) {
        // Add click event for payment
        ...
        // Add tooltip
        cartIcon.title = 'View Cart & Checkout';
    }
}
