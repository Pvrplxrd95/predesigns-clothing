...
class ProductManager {
    ...
    /**
     * Initialize image hover galleries
     */
    initImageGalleries() {
        document.querySelectorAll('.product-card').forEach(card => {
            const main = card.querySelector('.main-image');
            const secondary = card.querySelector('.secondary-image');
            if (main && secondary) {
                const originalSrc = main.getAttribute('src');
                card.addEventListener('mouseenter', () => main.src = secondary.src);
                card.addEventListener('mouseleave', () => main.src = originalSrc);
            }
        });
    }
        ...
        document.querySelectorAll('.add-to-wishlist').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (window.showCartNotification) {
                    window.showCartNotification(`${name} ${isAdded ? 'added to' : 'removed from'} wishlist`, isAdded ? 'success' : 'info');
                }
            });
        });
    }
