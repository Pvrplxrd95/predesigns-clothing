...
class AuthManager {
    ...
    getAuthModalHTML(activeTab) {
        ...
        return `
            <div class="modal-content">
                ...
                <div id="auth-content">
                    ${activeTab === 'login' ? loginHTML : registerHTML}
                </div>
            </div>
        `;
    }
    ...
    // Show login tab
    showLoginTab() {
        const loginHTML = this.getAuthModalHTML('login');
        const match = loginHTML.match(/<div id="auth-content">([\s\S]*?)<\/div>/);
        if (match && match[1]) {
            document.getElementById('auth-content').innerHTML = sanitizeHTML(match[1]);
        }
    ...
    // Show register tab
    showRegisterTab() {
        const registerHTML = this.getAuthModalHTML('register');
        const match = registerHTML.match(/<div id="auth-content">([\s\S]*?)<\/div>/);
        if (match && match[1]) {
            document.getElementById('auth-content').innerHTML = sanitizeHTML(match[1]);
        }
